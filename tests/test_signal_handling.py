...
def test_setup_signal_handlers_posix() -> None:
    ...
    with patch("sys.platform", "linux"), \
        ...
        # Verify it registers SIGTERM and SIGINT
        ...
        # First call should raise KeyboardInterrupt
        with pytest.raises(KeyboardInterrupt):
            handler(signal.SIGINT, None)

        # Second call should return None (ignored)
...
def test_setup_signal_handlers_win32() -> None:
    ...
    with patch("sys.platform", "win32"), \
         patch("signal.signal") as mock_signal:
        _setup_signal_handlers()
        # On Win32 we only register SIGINT
        sig_calls = [call[0][0] for call in mock_signal.call_args_list]
        assert signal.SIGINT in sig_calls
        assert signal.SIGTERM not in sig_calls
